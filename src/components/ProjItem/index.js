...
function ProjItem(props) {
    ...
    const checkButtons = () => {
        if (props.github == true && props.more == true) {
            return (
                <div class='pt-3'>
        ...
        else if (props.github==true) {
            return (
                <div class='pt-3'>
                    <a href={props.gitlink} target='_blank' rel='noopener noreferrer'><button><FontAwesomeIcon icon={faGithub} size='2x'></FontAwesomeIcon></button></a>
                </div>
            )
        }

        else if (props.more === true) {
            <div class='pt-3'>
    ...
    return(
        <div class='mx-auto w-3/4 mt-10 h-80 bg-green-500 rounded'>
            <div id='content' class='py-5 px-5 flex w-full h-full'>
                <div class='w-2/5 bg-blue-200 h-full'></div>
                <div class='w-3/5 px-3'>
                    <h1 class='font-bold font-sans text-3xl'>{props.title}</h1>
                    <h3 class='font-sans text-lg font-normal leading-snug'>Technologies used: {props.techstack}</h3>
